...
export class Reversi {
	...
	public possibleMoves(player: Player = this.turn): Move[] {
		...
		for (let y = 1; y < 9; y++) {
			for (let x = 1; x < 9; x++) {
				...
				if (
					this.board[y][x] === Square.blank &&
					this.canPlaceStone(x, y, player)
				) {
					ret.push({ x, y });
				}
			}
	...
	public countTurnableStones(
		...
		const getValueAt = (c: number) =>
			this.board[y + c * yd][x + c * xd] as number;
